...
; ---------------------------------------------------------------------
; Code hacked to work with the super NT by Kevtris
; 073117
;
; Changes:
;
...
;   CIC clk in (56) [7] |2  A5 A0 13| * Programming
;                D4 out |3  A4 A1 12| * Programming
;                /RST * |4  A3 A2 11| host reset out [10]
;           Region in * |5  C5 C0 10| CIC data i/o 0 (55) [1] / {50/60Hz in}
;   Disable Link Mode * |6  C4 C1  9| CIC data i/o 1 (24) [2] / {D4 in}
;          Key Detect * |7  C3 C2  8| CIC slave reset out (25) [11]
;                       `-----------'
; 
; (4) /RST:
; ---------
...
; (5) Region in:
; --------------
; Low = 60Hz, High = 50Hz.  This is set to the system's selected region.  
;
; (6) Disable Link Mode:
; ----------------------
; High = disable link mode.
;
; (7) Key Detect:
; ---------------
; High = Key in cart is connected and working
;
; (12, 13) Programming:
; ---------------------
; Used for programming so we don't use these for anything.
;
; The newer version of MPASM changes how banksel works.  This really was a huge pain, because I didn't know it.
; Comparing the hex files between the zip and my assembled version revealed that the assembled version was quite a bit smaller.
; Further inspection shows that it has two instructions for every banksel, because the assembler was buggy.  It was emitting a BCF for the upper
; bank bit on the status register, but the 630 does not have two bank bits, it only has one.  The later version of the assembler fixes this.,
; I have added NOPs to each banksel to fix this. 
;
; Changes are denoted by *K*.
; 
; This is a horrible hack.  I have very little time to clean it up.  I got it working properly for the project so that's what I am going with.  It has been
; tested with NTSC games, PAL games, and SA-1 games.  
...
rst				; we jump here after powerup or RC0=1
	...
	bcf	PORTC, 3	; *K* disable "lockout detected"
	bcf	PORTA, 2	; hold the SNES in reset
	movlw	0x30		; prescaler 1:8
	movwf	T1CON		;
	clrf	PIR1
rst_loop
	btfss	PORTA, 3	; *K* reset moved and inverted stay in "reset" as long as RA0=1
	goto	rst_loop
	...
	movlw	h'01'		; *K* set the default region from pin state
	btfsc	PORTC, 5
	movlw	h'02'
...
;	PORTC: out out  in out out  in
	...
	;movwf	TRISC
	movlw	0x2b		; *K* in out in out in in
	movwf	TRISA
	movlw	0x31		; *K* in in out out out in
	movwf	TRISC	
...
loop1
	...
	;movf	0x20, w
	movf	0x20, w		; changed how this works
	iorwf	0x50, w
	nop
	movwf	PORTC
	nop
	...
	bsf	0x59, 4
	;nop
	bcf	0x50, 3		; lockout is not working
...
scic_pair_skip2	
	...
	nop
	nop
	nop			; *K* removed to account for the btfss
...
die
	btfss	PORTA, 3	; *K* moved and inverted
	goto	rst
	goto	die
...
rst2
	...
	clrf	0x51		; clear reset button state
rst2_loop1
	btfss	PORTA, 3	; if reset button is pressed
	goto	rst		; *K* invert and move
